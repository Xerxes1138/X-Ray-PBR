...
void	CRenderTarget::phase_combine	()
{
	...
	Fmatrix		m_CameraProjection;
	m_CameraProjection.set(Device.mProject);

	Fvector4	projectionParams = {VIEWPORT_NEAR, 350.0f,0.0f,0.0f};

	// 
	RCache.set_c				("dx_matrix_CameraProjection",		m_CameraProjection	);
	RCache.set_c				("dx_ProjectionParams",		projectionParams	);
	...
	if (!_menu_pp)
	{
		...
		const float minamb			= 1e-5f;

		// Removed all mul by 2 stuff from here and also in Environment, Environment_Misc and Environment_Render.
		...
		Fvector4	ambclr			= {_max(envdesc.ambient.x, minamb),	_max(envdesc.ambient.y, minamb), _max(envdesc.ambient.z, minamb), 0.0f};
					ambclr.mul(ps_r2_sun_lumscale_amb);

		Fvector4	envclr			= {envdesc.sky_color.x + EPS, envdesc.sky_color.y + EPS, envdesc.sky_color.z + EPS,	envdesc.weight};
	...
	// Forward rendering
	{
		...
		//RImplementation.render_forward	(); // BAsically renders forward shader
	...
	if (0)	
	{
		...
		{
			RCache.set_Element	(s_combine->E[/*bDistort*/ 0? 3 : 1]);	// look at blender_combine.cpp
		}
		else				
		{
			RCache.set_Element	(s_combine->E[/*bDistort*/0 ? 4 : 2]);	// look at blender_combine.cpp
		}
