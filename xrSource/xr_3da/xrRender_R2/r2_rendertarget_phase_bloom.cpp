...
void CRenderTarget::phase_bloom	()
{
	...
	// Targets
	u_setrt									( rt_Bloom_1,NULL,NULL,NULL,NULL );		// No need for ZBuffer at all
	...
	if (ps_r2_ls_flags.test(R2FLAG_FASTBLOOM))
	{
		...
		// P0
		u_setrt						(rt_Bloom_2,NULL,NULL,NULL,NULL);			// No need for ZBuffer at all
...
/*void CRenderTarget::phase_bloom	()
{
	...
	if(1)
	{
		...
		struct v_postprocess_bloom_prepass
		{
			Fvector4	position;
			Fvector2	uv0;
		};
		...
		// Fill vertex buffer
		v_postprocess_bloom_prepass* pv = (v_postprocess_bloom_prepass*) RCache.Vertex.Lock(4, g_postprocess_bloom_prepass->vb_stride, Offset);
		...
		RCache.Vertex.Unlock(4, g_postprocess_bloom_prepass->vb_stride);

		RCache.set_Shader	(s_postprocess_bloom_prepass);
		RCache.set_Geometry			(g_postprocess_bloom_prepass);
		RCache.Render				(D3DPT_TRIANGLELIST,Offset,0,4,0,2);
	...
	if(1)
	{
		...
		struct v_postprocess_stochastic_bloom
		{
			Fvector4	position;
			Fvector2	uv0;
		};
		...
		// Fill vertex buffer
		v_postprocess_stochastic_bloom* pv = (v_postprocess_stochastic_bloom*) RCache.Vertex.Lock(4, g_postprocess_stochastic_bloom->vb_stride, Offset);
		pv->position.set(EPS, float(_h + EPS), EPS, 1.f);
		pv->uv0.set(p0.x, p1.y);
		pv++;
		...
		RCache.Vertex.Unlock(4, g_postprocess_stochastic_bloom->vb_stride);

		RCache.set_Shader	(s_postprocess_stochastic_bloom);
		RCache.set_Geometry			(g_postprocess_stochastic_bloom);
		RCache.Render				(D3DPT_TRIANGLELIST,Offset,0,4,0,2);
