...
void CRender::render_menu	()
{
	...
	// Main Render
	{
		Target->u_setrt						(Target->rt_Generic_0,0,0,0,HW.pBaseZB);		// LDR RT
		g_pGamePersistent->OnRenderPPUI_main()	;	// PP-UI
	}
	// Distort
	{
		Target->u_setrt						(Target->rt_Generic_1,0,0,0,HW.pBaseZB);		// Now RT is a distortion mask
...
//.	VERIFY					(g_pGameLevel && g_pGameLevel->pHUD);
	...
	// !EXPERIMENTAL! if (ps_r2_ls_flags.test(R2FLAG_EXP_SPLIT_SCENE))split_the_scene_to_minimize_wait = TRUE; // !EXPERIMENTAL!
	...
	// SSSSS
	// TODO 
	// Take the diffuse accumulator pass, perfom the sss blur then update the diffuse accumulator pass
	//
	...
	// TONEMAP
	Target->phase_postprocess_tonemap		();
	//
