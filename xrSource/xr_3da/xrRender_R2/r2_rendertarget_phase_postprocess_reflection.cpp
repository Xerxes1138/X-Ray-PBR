...
void CRenderTarget::phase_postprocess_reflection()
{
	...
	if(1)
	{
		...
		struct v_postprocess_ssr_resolve
		{
			Fvector4	position;
			Fvector2	uv0;
		};
		...
		// Fill vertex buffer
		v_postprocess_ssr_resolve* pv = (v_postprocess_ssr_resolve*) RCache.Vertex.Lock(4, g_postprocess_ssr_resolve->vb_stride, Offset);
		pv->position.set(EPS, float(_h + EPS), EPS, 1.f);
		pv->uv0.set(p0.x, p1.y);
		pv++;
		...
		RCache.Vertex.Unlock(4, g_postprocess_ssr_resolve->vb_stride);

		RCache.set_Shader	(s_postprocess_ssr_resolve);
		...
		u_setrt	(rt_ssr_resolve, NULL, NULL, NULL, HW.pBaseZB);
		RCache.set_ColorWriteEnable	();
		RCache.set_CullMode( CULL_NONE );
		RCache.set_Stencil(FALSE);

		RCache.set_Geometry			(g_postprocess_ssr_resolve);
		RCache.Render				(D3DPT_TRIANGLELIST,Offset,0,4,0,2);
	}
	...
	//}
		...
		u_setrt	(rt_ssr_resolve, 0, 0, 0, HW.pBaseZB);
		RCache.set_CullMode(CULL_NONE);
		RCache.set_Stencil(FALSE);
