...
CRenderTarget::CRenderTarget		()
{
	...
	g_blit.create(D3DFVF_XYZRHW|D3DFVF_TEX1, RCache.Vertex.Buffer(), RCache.QuadIB);
	...
	//	NORMAL
	{
...

		{
			...
			u32 ssrRaycastWidth = w;
			u32 ssrRaycastHeight = h;
		...
		rt_AccumulatorSpecular.create(r2_RT_accumSpecular,w,h, D3DFMT_A16B16G16R16F);

		rt_distortion.create			(r2_RT_distortion, w, h, D3DFMT_A16B16G16R16F);
	...
	// BLOOM
	{
		...
		g_postprocess_bloom_prepass.create(D3DFVF_XYZRHW|D3DFVF_TEX1, RCache.Vertex.Buffer(), RCache.QuadIB);

		s_postprocess_stochastic_bloom.create(b_postprocess_stochastic_bloom);
		g_postprocess_stochastic_bloom.create(D3DFVF_XYZRHW|D3DFVF_TEX1, RCache.Vertex.Buffer(), RCache.QuadIB);
	...
	if(1)
	{
		s_postprocess_tonemap_exposure.create(b_postprocess_tonemap_exposure);
		g_postprocess_tonemap_exposure.create(D3DFVF_XYZRHW|D3DFVF_TEX1, RCache.Vertex.Buffer(), RCache.QuadIB);

		s_postprocess_tonemap.create(b_postprocess_tonemap);
		g_postprocess_tonemap.create(D3DFVF_XYZRHW|D3DFVF_TEX1, RCache.Vertex.Buffer(), RCache.QuadIB);
	...
	// COMBINE
	{
		...
		g_combine_VP.create					(D3DFVF_XYZRHW|D3DFVF_TEX1,		RCache.Vertex.Buffer(), RCache.QuadIB);
