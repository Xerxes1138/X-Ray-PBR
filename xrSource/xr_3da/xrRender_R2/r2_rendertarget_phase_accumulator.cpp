...
void CRenderTarget::phase_accumulator()
{
	...
	if (dwAccumulatorClearMark==Device.dwFrame)
	{
		...
		if (RImplementation.o.fp16_blend)	u_setrt	(rt_Accumulator,		rt_AccumulatorSpecular,NULL,NULL,HW.pBaseZB);
		else								u_setrt	(rt_Accumulator_temp,	rt_AccumulatorSpecular,NULL,NULL,HW.pBaseZB);
	} 
	else
	{
		// initial setup
