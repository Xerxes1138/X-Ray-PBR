...
#include "..\xrSky\ArHosekSkyModel.h"
    ...
    static float IlluminanceIntegral(float theta)
    {
	 ...
	 Fvector SunLuminance(bool& cached)
    {
		Fvector sunDirection = {0.0f, 0.5f, 1.0f};
		clamp(sunDirection.y, 0.0f, 1.0f);
	
		sunDirection.normalize();

         float turbidity = 1.0f;
		clamp(turbidity, 1.0f, 32.0f);

        float intensityScale = 1;
        Fvector3 tintColor = {1.0f, 1.0f, 1.0f};

        const bool normalizeIntensity = true;
        const float sunSize = 20.0f;

        static float turbidityCache = 2.0f;
		static Fvector3 sunDirectionCache = {-0.579149902f, 0.754439294f, -0.308879942f};

		static Fvector3 luminanceCache = {1.61212531e+009f, 1.36822630e+009f, 1.07235315e+009};

        static Fvector3 sunTintCache =  {1.0f, 1.0f, 1.0f};
        static float sunIntensityCache = 1.0f;
        static bool normalizeCache = false;
        static float sunSizeCache = 20;

        /*if(turbidityCache == turbidity && sunDirection == sunDirectionCache && intensityScale == sunIntensityCache && tintColor == sunTintCache && normalizeCache == normalizeIntensity && sunSize == sunSizeCache)
        {
            cached = true;
       ...
       /* cached = false;*/
        ...
        for(uint64 x = 0; x < NumDiscSamples; ++x)
        {
            for(uint64 y = 0; y < NumDiscSamples; ++y)
            {
                ...
                Fvector3 sampleRadiance = solarRadiance.ToRGB() * FP16Scale;
...
void CRenderTarget::accum_direct		(u32 sub_phase)
{
	...
	if (SE_SUN_NEAR==sub_phase)	//.
	{
		...
		RCache.set_c				("Ldynamic_dir",		dir.x,dir.y,dir.z,0		);
			RCache.set_c				("Ldynamic_dir_w",		fuckingsun->direction.x,fuckingsun->direction.y,fuckingsun->direction.z,0		);
...
void CRenderTarget::accum_direct_f		(u32 sub_phase)
{
	...
	phase_accumulator					();
	u_setrt								(rt_Generic_0,NULL,NULL,NULL,HW.pBaseZB);
	...
	// Perform lighting
	{
		u_setrt								(rt_Generic_0,NULL,NULL,NULL,HW.pBaseZB);  // enshure RT setup
		RCache.set_CullMode					(CULL_NONE	);
		RCache.set_ColorWriteEnable			();
