...
void	CBlender_Detail_Still::Compile	(CBlender_Compile& C)
{
	...
	case SE_R2_NORMAL_HQ: 		// deffer wave
		uber_deffer				(C,true,"detail_w","base",true);
		break;
	...
	case SE_R2_SHADOW:
		if (bAref)	
			C.r_Pass	("shadow_direct_detail_s_flat","shadow_direct_base_aref",	FALSE,TRUE,TRUE,TRUE,D3DBLEND_ZERO,D3DBLEND_ONE,TRUE,200);
		else				
			C.r_Pass	("shadow_direct_detail_s_flat","shadow_direct_base",		FALSE);
