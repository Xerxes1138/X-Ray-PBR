...
void CEffect_Thunderbolt::OnFrame(int id, float period, float duration)
{
	...
	if (state==stWorking){
      ...
      //  g_pGamePersistent->Environment().CurrentEnv.sky_color.mad(fClr,p_sky_color); // Affect the sky box color, was here to simulate a lightning illumination in the sky but since I remove _Power it resulted in pitch black 
